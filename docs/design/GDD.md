...
**El mapa como menú principal (v1.3).** Cada Inmersión es un nodo de un mapa vertical al estilo *Super Mario Bros.* (diseño completo en `docs/design/WORLD-MAP.md`): el mapa **es** el menú principal, no una pantalla de inicio aparte. Tocar un nodo disponible arranca la Inmersión correspondiente desde la estación anterior, exactamente como antes hacía la pantalla de inicio. La estación de descanso (§3.3) conserva intacto el botón gigante **"Seguir bajando"** —la campaña sigue siendo un solo desplome, nunca una vuelta forzada al menú— y añade uno pequeño **"Mapa"** para quien prefiere cerrar la sesión ahí; el menú de pausa hace lo mismo desde "Salir". La primera partida nunca ve el mapa: entra directa a la Inmersión 1 como hoy, porque el §8 prohíbe cualquier modal antes del tutorial mudo; el mapa aparece por primera vez al llegar a la primera estación, y desde ahí el juego arranca siempre en él.
...
**Valoración de tienda: fuera del pico y detrás de una puerta parental.** La v1.0 pedía la valoración a un niño de 7–12 años en el momento de máxima euforia, que es a la vez lo menos defendible y lo que un revisor de políticas mira primero. Regla nueva: la petición **no aparece nunca dentro del descenso ni en una entrega de Postal**; vive en el **menú principal** —desde v1.3, el mapa del mundo (§3.1)—, solo tras completar la Zona 3, **una sola vez**, y **detrás de la puerta parental de §6.6** (la misma que protege anuncios y enlaces externos). Si la puerta no se resuelve, no se pide y no se vuelve a ofrecer. Sigue sin implementarse: el mapa reserva el hueco pero no pide nada todavía.
...
- **Menú principal: el mapa (v1.3).** No hay pantalla de inicio aparte: el mapa vertical desplazable **es** el menú principal (diseño completo en `docs/design/WORLD-MAP.md`). Título arriba; una fila de islas por mundo (el Océano de Ámbar activo, Volcán y Lago Ness como siluetas bloqueadas con candado); un camino serpenteante de burbujas-nodo para las 18 Inmersiones del Océano de Ámbar, cada tramo teñido con la paleta de su zona (§3.2). Estado de cada nodo: **sin contenido** (gris, interrogante), **bloqueado** (candado), **disponible** (número, brillo si es el más profundo alcanzable) y **completado** (lleno, con sus 0–3 conchas debajo). Objetivos táctiles ≥ 44 pt; tocar un nodo bloqueado o sin contenido hace un pequeño gesto de "no", sin modal. Arranca centrado en el nodo más profundo desbloqueado. Sus palabras en pantalla (título y nombres de mundo) cuentan contra el presupuesto de <40 del pilar 5 (§0).
...
## 15. Registro de revisión (v1.2 → v1.3)

El *owner* pidió un menú principal con mapa del mundo al estilo *Super Mario Bros.* La propuesta completa vive en `docs/design/WORLD-MAP.md`; aquí solo el resumen que toca el GDD.

24. **El mapa sustituye a la pantalla de inicio.** Ya no hay una pantalla separada de "Seguir · N m / Desde la superficie": el mapa **es** el menú principal, y tocar el nodo más profundo desbloqueado o el nodo 1 hace lo mismo que hacían esos dos botones. §3.1, §8.
25. **Nivel de mapa = Inmersión.** Cada nodo es una Inmersión (18 en el Océano de Ámbar); el área/zona solo tiñe el tramo del camino, no es una unidad de desbloqueo. Respuesta por defecto de `docs/design/WORLD-MAP.md` §6.1, aprobada por el owner. §3.1.
26. **La estación no fuerza la vuelta al mapa.** Se mantiene "Seguir bajando" como botón gigante por defecto y se añade "Mapa" como opción pequeña, en vez de volver siempre al mapa como en *Mario*. Respuesta por defecto §6.2, aprobada. §3.1, §3.3.
27. **La primera partida no ve el mapa.** Arranca directa en la Inmersión 1, como hoy; el mapa aparece por primera vez al llegar a la primera estación. Respuesta por defecto §6.3, aprobada; consistente con la prohibición de modales antes del tutorial mudo. §3.1, §8.
28. **La valoración de tienda sigue reservada, no implementada**, y su sitio pasa a ser explícitamente el mapa. §3.3.
29. **`level/worlds.ts` en `core`** es la única fuente de verdad del desbloqueo (`worlds()`, `levelStatuses()`); `MapScene` en el shell solo la pinta. Mundos futuros (Volcán, Lago Ness) existen como huecos de datos bloqueados, sin diseño propio todavía.
